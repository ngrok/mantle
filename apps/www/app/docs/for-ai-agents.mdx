...
- If the user asks for "a primary button," generate a `Button` with `appearance="filled" intent="accent"`. `Button` and `IconButton` require explicit `appearance` and `intent` — there are no defaults. `intent` is the tone (`accent`, `danger`, or `neutral`); when no tone is implied, use `intent="accent"` for buttons and `intent="neutral"` for icon buttons.
