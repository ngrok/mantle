...
- Never render a conditional element immediately before bare text
  ...
  filenames, IDs, and passcodes translate="no".
...
- If the user asks for "a primary button," generate a `Button` with `appearance="filled" intent="neutral"` — the system's default primary action. `Button` and `IconButton` require explicit `appearance` and `intent` — there are no defaults. `intent` is the tone (`accent`, `danger`, or `neutral`); when no tone is implied, use `intent="neutral"` for both buttons and icon buttons, reserving `intent="accent"` for deliberate brand emphasis (an action that should carry the accent color). `IconButton` narrows the union to `"neutral"` alone, so reach for `Button` where the tone carries meaning.
