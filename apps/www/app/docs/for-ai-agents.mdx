...
| [`/api/components.json`](/api/components.json)     | Structured component manifest. Each entry has `name`, `slug`, `kind`, `category`, `status`, `importPath`, `docsUrl`, `markdownUrl`, and a one-line `summary`.  |
...
| [`/layouts`](/layouts)                             | Published primitives that own page/viewport structure (app shells, centered flows). New section — the first residents are still graduating from incubation.    |
