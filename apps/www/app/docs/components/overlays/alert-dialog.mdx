...
A button that confirms the Alert Dialog action. Defaults to `appearance="filled"`, with a tone derived from `AlertDialog.Root`'s `intent`: `"danger"` dialogs get a danger button, `"info"` dialogs get a neutral one — the system's default primary action. Does not close the alert dialog by default.
