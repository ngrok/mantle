...
Use `AlertDialog` for destructive or irreversible confirmations (delete, sign out, leave without saving) where the user must explicitly acknowledge. `AlertDialog` blocks all background interaction. The content carries `role="alertdialog"`, and a click on the backdrop does not close it: the user closes it through `AlertDialog.Cancel`, `AlertDialog.Close`, or the Escape key. For non-destructive input modals, use [`Dialog`](/components/overlays/dialog).
...
| Prop              | Type                    | Default      | Description                                                                   |
| ----------------- | ----------------------- | ------------ | ----------------------------------------------------------------------------- |
| `preferredWidth?` | `` `max-w-${string}` `` | `"max-w-md"` | The preferred width of the `AlertDialogContent` as a Tailwind `max-w-` class. |

`onInteractOutside` still fires, but `AlertDialog.Content` calls `event.preventDefault()` after your handler, so an outside pointer interaction never closes the dialog.
...
An icon that visually represents the intent of the AlertDialog. Defaults to a warning icon for danger intent and an info icon for info intent with the appropriate color. The icon is decorative and renders `aria-hidden="true"` unless you name it.

All props from [svg](https://developer.mozilla.org/en-US/docs/Web/SVG/Element/svg#attributes) except `children`, plus:

| Prop   | Type        | Default                     | Description                                                          |
| ------ | ----------- | --------------------------- | -------------------------------------------------------------------- |
| `svg?` | `ReactNode` | An icon derived from intent | A single SVG icon element that renders in place of the default icon. |
