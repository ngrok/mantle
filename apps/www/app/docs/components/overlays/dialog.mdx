...
| `onPointerDownOutside?` | `(event: PointerDownOutsideEvent) => void`      |              | Event handler called when a pointer event starts outside the component. It can be prevented by calling `event.preventDefault`. See [Handling autofill overlays](#handling-autofill-overlays). |
