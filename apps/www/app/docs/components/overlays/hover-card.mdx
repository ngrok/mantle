...
- Prefer [`Tooltip`](/components/overlays/tooltip) for short, non-interactive labels on controls.
- Prefer [`Popover`](/components/overlays/popover) when the content must be reachable by all users and may contain interactive elements.
...
| Prop         | Type                                  | Default                 | Description                                                                                                                                                       |
| ------------ | ------------------------------------- | ----------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `container`  | `Element \| DocumentFragment \| null` | nearest layer container | The container element to portal the content into. When omitted, the content mounts into an enclosing overlay's positioner when one is open, else `document.body`. |
