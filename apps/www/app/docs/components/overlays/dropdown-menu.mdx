...
An icon you pass as a child is now a grandchild of the item, so a `[&>svg]` class of your own no longer reaches it. Write `[&_svg]` instead, or put the size class on the icon itself.
