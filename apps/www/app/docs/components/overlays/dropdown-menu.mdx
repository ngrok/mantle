...
Three parts do **not** take it, and the prop is omitted from their props types:
`DropdownMenu.SubTrigger`, `DropdownMenu.CheckboxItem`, and `DropdownMenu.RadioItem`. Each renders a second
element beside your children — the submenu caret, or the check indicator — so a slot would receive two
elements and throw. Style them with `className` instead.
