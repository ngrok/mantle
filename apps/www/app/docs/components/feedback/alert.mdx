...
`Alert.Root` renders a plain `<div>` with no ARIA role. A static alert that is on the page at load does not need one. When your code inserts an alert after an action, such as "Invalid email or password." after a submit, pass `role="alert"` so a screen reader announces it. Pass `role="status"` for a less urgent message that waits until the user is idle.
...
Displays a callout for user attention. Root container for all `Alert` sub-components. It has no ARIA role of its own: pass `role="alert"` when your code inserts the alert after an action, so a screen reader announces it.
