...
function EmptyInTableExample({ data }: { data: Row[] }) {
				...
				{rows.length > 0 ? (
					...
					<DataTable.EmptyRow>
						<Empty.Root>
							...
							<Empty.Actions>
								<Button type="button" appearance="outlined" intent="neutral" onClick={clearFilters}>
									Clear filters
								</Button>
							</Empty.Actions>
						</Empty.Root>
...
An empty state that sits over anything but a flat surface needs `Empty.Scrim`. Compose it as the first child of `Empty.Root`: it paints an ellipse of the surface color that fades to nothing before the container's edge, it takes no space, and it stays out of the accessibility tree.

The reason is measured, not decorative. `Empty.Description` sits on `text-muted`, which measures 4.88:1 on a bare light card — 0.38 over the 4.5:1 floor for 14px text. Anything tinted under that text spends the headroom and drops the description under WCAG AA. A [decorative chart](/components/charts/bar-chart#decorative-charts) recessed to 70% opacity leaves the worst chart slot at 2.74:1. The scrim restores the flat surface locally, so the message measures what it would on a bare card.

Set `--empty-scrim-color` when the empty state sits on a surface other than a card, so the wash repaints the right color. A mismatch reads as a lighter or darker patch behind the copy, and dark mode shows it first: the card and page surfaces sit 2.3 lightness points apart there, against 0.4 in light mode.
...
A soft wash behind the content, so the copy stays readable over a busy backdrop. Compose it as the first child of `Empty.Root`.

The scrim is paint and nothing else. It is `aria-hidden`, never a tab stop, inert to pointer events, and it takes no space. Those hold even when you spread your own values over the part, so a stray `tabIndex` cannot strand focus on a shape a screen reader will not announce. It takes no children either — content nested inside an `aria-hidden` subtree goes missing from the accessibility tree.

`aria-disabled` is deliberately absent. It describes a widget a user can perceive but not operate, and the scrim is neither a widget nor perceivable once `aria-hidden` removes it.

All props from [div](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/div#attributes), except `children`.

| CSS Variable          | Default                        | Description                                                                                                                    |
| --------------------- | ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------ |
| `--empty-scrim-color` | `var(--background-color-card)` | The color the wash paints. Set it to the surface the empty state sits on — `var(--background-color-popover)` inside a popover. |
