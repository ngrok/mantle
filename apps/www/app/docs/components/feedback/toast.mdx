...
`Toast.Root` does not take `asChild`. The root renders the intent accent bar alongside your children, so a slot would receive two elements. Style the toast surface with `className` instead.
