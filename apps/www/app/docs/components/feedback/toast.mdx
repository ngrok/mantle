...
`Toast.Root` does not take `asChild`. The root renders the intent accent bar alongside the label div holding your children, so a slot would receive two elements. Style the toast surface with `className` instead.
...
`children` render inside a `<div data-slot="toast-label">`. The intent accent bar is a permanent element sibling, so without the wrapper a bare text child is never a lone child: a browser translation engine reparents that text node, and the removal React runs when the content changes shape or goes away throws. The wrapper is a `<div>` because `Toast.Message` renders a `<p>`, which a `<span>` may not contain. It is `display: contents`, so the icon, the message, and an action stay flex items of the root and `Toast.Message`'s `flex-1` still resolves. [Browser translation](/browser-translation) covers the mechanism and the rules your own components need.
