...
You can compose additional visual or functional elements within the `Input` using `children`. The examples below show you how to render start and end icons or buttons. The [Password Input](/components/forms/password-input) is built using this API under the hood! Keep in mind that you will need to manually pass the `InputCapture` component as children too because it is responsible for rendering the actual form `input` element! We provide an `InputCapture` component for you when you don't use the `children` API.
