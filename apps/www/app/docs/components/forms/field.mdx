...
export const WithoutToErrorMessagesExample = () => {
	const schema = z.object({
		email: z.email("Enter a valid email."),
	});
	const form = useForm({
...
`Field.Label` wraps the mantle [`Label`](/components/forms/label) for ergonomic field composition. It keeps the same click-to-focus, disabled, and typography behavior as `Label`; see the [Label docs](/components/forms/label) for the full surface.

**Label-to-control association is automatic inside a `Field.Item`.** `Field.Item` generates a stable control id. `Field.Control` splats that id onto its focusable child, and `Field.Label` points `htmlFor` at it. The required `name` on `Field.Item` is the single source of truth for the control's form name. That lets you write `<Field.Label>Email</Field.Label>` next to `<Field.Control><Input /></Field.Control>` with no matching `htmlFor` / `id` pair and no separate `useId()` call. To set the id yourself, pass `id` on `Field.Item`, for example when a Playwright locator or an anchor link must find the control by id. `Field.Control` and `Field.Label` both follow it, and the wrapping `<div>` carries no id. `Field.Label` has no `htmlFor` prop and `Field.Control` has no `id` prop, so `Field.Item` is the one owner. Render the focusable element inside `Field.Control` so the id lands on it.

Wrap the focusable control in `Field.Control` so `Field.Item` can associate descriptions and rendered errors via `aria-describedby` / `aria-errormessage`. A rendered `Field.Errors` or `Field.ErrorList` infers error state once the page hydrates. When you need to override that state, or when the error must be in the server HTML, pass `validation` on `Field.Item` (see [Server-rendered errors](#server-rendered-errors)).

**`Field.Item` owns the full control contract.** When a focusable control is wrapped in `Field.Control`, the surrounding `Field.Item` owns its `id`, `name`, `aria-describedby`, `aria-errormessage`, `aria-invalid`, and `validation`; anything supplied on the child element (or on `Field.Control`) is overwritten. There's one place to override: `Field.Item` itself, with `id`, `name`, or `validation`. To opt out of the contract entirely, render the control without `Field.Control`.
...
<Example className="w-full">
	<Field.Item name="recoveryCode" className="max-w-80">
		<Field.Label>Recovery code</Field.Label>
		<Field.Control>
			<Input />
		</Field.Control>
...
Use `Field.Errors` below the control when validation fails. It accepts string messages, trims them, filters empty values, removes duplicates, and renders a semantic `<ul>` with `<li>` items so screen readers announce the errors as a list. When the list renders, `Field.Item` infers error state after hydration so the visual state and ARIA invalid state stay in sync. For errors that render on the server, see [Server-rendered errors](#server-rendered-errors) below.

For custom list markup or rich error content, compose the lower-level `Field.ErrorList` and `Field.ErrorItem` parts directly. A single error is still a list of one.
...
`Field.Item` infers the `"error"` state on the client: a mounted `Field.Errors` or `Field.ErrorList` registers with `Field.Item` after hydration. On the server, nothing runs that registration. The server HTML carries the error list. Until the page hydrates, the control is not flagged `aria-invalid="true"` and has no `aria-errormessage`, and the item has no `data-validation="error"`. The control keeps its neutral border. A screen reader doesn't announce it as invalid.

When errors come back from the server, for example from a React Router action, set `validation` on `Field.Item` from the same messages. An explicit `validation` is in the server HTML from the first paint:
...
`validation` accepts `false`, so the short-circuit yields `false` with no messages and `"error"` with at least one. Client-only validators, such as the TanStack Form example below, don't need this: they run after hydration, where the inference already works.
...
`Field.Label` wraps [`Label`](/components/forms/label) and always points `htmlFor` at the surrounding `Field.Item`'s control id. `htmlFor` is not a prop; set `id` on `Field.Item` to choose the id. Use it inside `Field.Item` to give the control an accessible name and click-to-focus behavior.

See the [Label docs](/components/forms/label) for association patterns, disabled state, and typography behavior.

All props from [`Label`](/components/forms/label) except `htmlFor`.
...
<Example className="w-full">
	<Field.Item name="owner" className="max-w-80">
...
A single form field. Renders a plain `<div>` with `flex flex-col gap-1.5` so `Field.Label`, control, and helper / error siblings stack tightly together. No implicit ARIA role: the `<label htmlFor>` ↔ control association already carries the right semantics for a single field. It owns the description/error IDs and validation state that `Field.Control` applies; rendered errors infer `"error"` after hydration unless `validation` is set. For a server-rendered error, set `validation` (see [Server-rendered errors](#server-rendered-errors)).
...
All props from [div](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/div#attributes) except `id`, plus:
...
| `asChild?`    | `boolean`                                    | `false` | Render as a different element by passing a child.                                                                                                                                                                |
| `id?`         | `string`                                     | —       | DOM id for the focusable control, not for the wrapping `<div>`. `Field.Control` splats it onto the control and `Field.Label` uses it as the default `htmlFor`. When omitted, `Field.Item` generates a stable id. |
| `validation?` | `"error" \| "success" \| "warning" \| false` | —       | Validation state that `Field.Control` applies. Overrides the inferred `"error"` state; set it for a server-rendered error.                                                                                       |

### Field.Control

Applies `Field.Item` description, error, identity, and validation state to a single control. It always behaves like an `asChild` slot: pass one child element to receive the generated props, or use a function child to place those props manually. Mantle compound controls that consume `FieldControlContext` can be wrapped at the root when their docs show that composition; otherwise use the render-prop form to place the generated props on the actual focusable element. Override validation on the surrounding `Field.Item`. `id` is not a prop here either; set it on `Field.Item`.

For the simple element-child form, see the [primary example](#field) above; to wrap a compound control (`Select`, `MultiSelect`) at its root, follow the composition shown in that control's own docs.
...
| `appearance?` | `"filled"` \| `"ghost"` \| `"outlined"` | `"ghost"`          | `IconButton` appearance variant.                                      |
| `intent?`     | `"neutral"`                             | `"neutral"`        | `IconButton` intent (tone). `IconButton` draws the neutral tone only. |
| `size?`       | `"xs"` \| `"sm"` \| `"md"`              | `"xs"`             | `IconButton` size.                                                    |
| `className?`  | `string`                                | —                  | Merged onto the underlying `IconButton`.                              |

### Field.HelpContent

Body of a `Field.Help` popover. Forwards every `Popover.Content` prop — see the [Popover docs](/components/overlays/popover#popovercontent) for the full surface (`side`, `align`, `preferredWidth`, etc.).
...
Convenience renderer for validation messages. Accepts an array of strings, trims each message, filters empty, nullish, and false values, then renders a `Field.ErrorList` with one `Field.ErrorItem` per remaining message. When rendered inside `Field.Item`, a non-empty list receives an automatic generated ID and infers `"error"` validation after hydration unless `Field.Item` overrides it. `Field.Control` merges that ID into the control's `aria-describedby` and `aria-errormessage` when the field is invalid. The ID is owned by `Field.Item` and is not consumer-overridable. For a server-rendered error, set `validation` on `Field.Item` (see [Server-rendered errors](#server-rendered-errors)).
...
A single error message for a field. Renders an `<li>` in `text-danger-600`, so it must be nested inside a `Field.ErrorList`. Empty or blank children render nothing. A single error is a list of one.

All props from [li](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/li#attributes).

### Field.ErrorList

Wraps one or more `Field.ErrorItem` children in a semantic `<ul>` with `role="list"` so a list of validation errors is announced as a list by screen readers, including Safari/VoiceOver combinations that drop list semantics when list styling is removed. Renders nothing when given no renderable children. When rendered inside `Field.Item`, a non-empty list receives an automatic generated ID and infers `"error"` validation after hydration unless `Field.Item` overrides it. `Field.Control` merges that ID into the control's `aria-describedby` and `aria-errormessage` when the field is invalid. The ID is owned by `Field.Item` and is not consumer-overridable. For a server-rendered error, set `validation` on `Field.Item` (see [Server-rendered errors](#server-rendered-errors)).
...
<Example className="w-full">
	<Field.Item name="email" className="max-w-80">
		...
		<Field.Errors messages={toErrorMessages(["Enter a valid email."])} />
	</Field.Item>
</Example>
