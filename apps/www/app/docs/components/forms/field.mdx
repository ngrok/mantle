...
`Field.Label` wraps [`Label`](/components/forms/label) and defaults `htmlFor` from the surrounding `Field.Item`. Use it inside `Field.Item` to give the control an accessible name and click-to-focus behavior.

See the [Label docs](/components/forms/label) for association patterns, disabled state, and typography behavior.

All props from [`Label`](/components/forms/label).
