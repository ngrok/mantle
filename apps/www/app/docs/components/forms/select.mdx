...
`Select.Trigger`, `Select.Content`, and `Select.Item` take no `asChild`, and their props types omit it. Each one renders its own chrome next to your content (the trigger's caret, the content's scroll buttons and viewport, the item's check indicator), so the underlying Radix `Slot` would receive more than the single child it requires and throw. To customize the trigger's content, pass it as `children` instead.
...
All props from Radix [Select.Trigger](https://www.radix-ui.com/primitives/docs/components/select#trigger), except `asChild`, plus:
...
All props from Radix [Select.Item](https://www.radix-ui.com/primitives/docs/components/select#item), except `asChild`, plus:
