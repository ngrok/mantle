...
`Select.Trigger`, `Select.Item`, `Select.Group`, `Select.Label`, and `Select.Separator` accept `asChild`, which swaps the part's default element for your single child while keeping its styling and the select's keyboard and selection behavior. `Select.Label asChild` helps when a group heading must be a specific element for the page's outline. `Select.Trigger asChild` makes your own control the combobox, and `Select.Item asChild` makes your own element the option. Each part renders its own chrome beside your children (the trigger's caret, the item's check indicator), so Radix clones your single child as the part's element and moves the label span and that chrome inside it. The [translation guarantee](/browser-translation) holds on both paths.

`Select.Root` is a state provider that renders no DOM, so it takes no `asChild`.

`Select.Content` takes no `asChild`, and its props type omits it. It renders the scroll buttons and the viewport around your items, so there is no single child for a slot to clone. Style it with `className` instead.
...
`children` render inside a `<span data-slot="select-trigger-label">`. The caret is a permanent element sibling, so without the span a bare text child is never a lone child: a browser translation engine reparents that text node, and the removal React runs when the child changes shape or goes away throws. The span is `display: contents`, so `Select.Value` stays a flex item of the trigger and keeps its `gap`. [Browser translation](/browser-translation) covers the mechanism and the rules your own components need.

`Select.Value` is now a grandchild of the trigger, so a `[&>span]` class of your own reaches the wrapper instead. Match the part's own variant: `[&>[data-slot=select-trigger-label]>span]:line-clamp-none`. That form shares the default's variant prefix, so tailwind-merge drops the default instead of shipping both. A bare `[&>span]:line-clamp-none` lands on the wrapper, and `line-clamp` sets `display`, which gives the wrapper a box and takes `Select.Value` out of the trigger's flex row.
...
All props from Radix [Select.Content](https://www.radix-ui.com/primitives/docs/components/select#content), except `asChild`, plus:
