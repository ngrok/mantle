...
Always pair with a [`Label`](/components/forms/label). The toggle is a focusable button with `aria-pressed`, named "Show value". The name does not change with state and does not contain the word "password", so a label query for the input matches one element. Its `aria-controls` points at the input's `id`: the `id` you pass, else a generated one. The input keeps `autocomplete="current-password"` / `"new-password"` semantics. Set `autoComplete` explicitly per flow.
