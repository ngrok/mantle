...
Always pair with a [`Label`](/components/forms/label). The toggle button has its own accessible name announcing the current state. The input keeps `autocomplete="current-password"` / `"new-password"` semantics — set `autoComplete` explicitly per flow.
