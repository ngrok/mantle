    	...
    	<div className="flex flex-col gap-2">
    		<Label className="flex items-center gap-2">
    			<Checkbox
    				...
    				checked={selectAllChecked({ allSelected, someSelected })}
    				onChange={(event) => setSelected(event.target.checked ? [...rows] : [])}
    			/>
    			Select all
    		</Label>
    		{rows.map((row) => (
    			<Label key={row} className="flex items-center gap-2 pl-6">
    				<Checkbox
    					checked={selected.includes(row)}
    					onChange={(event) =>
    						setSelected((prev) =>
    							event.target.checked ? [...prev, row] : prev.filter((value) => value !== row),
    						)
    					}
    				/>
    				{row}
...
For the fully-clickable list-row variant (and the full API), see the [`Choice` docs](/components/forms/choice).
...
| `readOnly?`       | `boolean`                                                                                                | `false` | Blocks the toggle and sets `aria-readonly`. The native `readonly` attribute is inert on a checkbox, so mantle guards the click itself.                                                                                                       |
...
For a checkbox with a titled, multi-line label and a description, see [`Choice`](/components/forms/choice).
