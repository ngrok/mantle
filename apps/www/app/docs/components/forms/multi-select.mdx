...
`MultiSelect.Item` takes no `asChild`, and its props type omits it. The item renders its own check indicator beside your children, so the slot would receive two elements and throw. To give an option a custom layout, pass it as `children` (a [`MediaObject`](/components/structure/media-object) with an avatar, say), which is unrestricted.
...
All props from ariakit [ComboboxItem](https://ariakit.org/reference/combobox-item), except `render` and `asChild`, plus:
