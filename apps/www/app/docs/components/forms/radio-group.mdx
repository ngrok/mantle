...
Use **`Choice.Title`** here, not `Choice.Label`: the whole `RadioGroup.Item` is the click target, and a `<label htmlFor>` can't associate with the radio's `role="radio"` element. Inside any radio item (`Item`, `ListItem`, `Card`, or `Button`), `Choice.Title` and `Choice.Description` register their ids with the radio. The radio's `aria-labelledby` points at the title, so the option's accessible name is the title alone, and its `aria-describedby` points at the description. Without that wiring, `role="radio"` would flatten both texts into one name. A plain `RadioGroup.ItemContent` has no title or description part to register, so its text still reads as one name; reach for `Choice` when the option carries helper text.
...
A simple radio item. The conventional use-case for basic radio options. Must be a child of `RadioGroup.Root`. Inside `Field.Control`, the item picks up `aria-invalid`, `aria-errormessage`, and `aria-describedby`. A `Choice.Title` inside names it through `aria-labelledby`, and a `Choice.Description` describes it through `aria-describedby`.
...
A radio button used inside `RadioGroup.ButtonGroup` for inline grouped radio options. Inside `Field.Control`, the item picks up `aria-invalid`, `aria-errormessage`, and `aria-describedby`. A `Choice.Title` inside names it through `aria-labelledby`, and a `Choice.Description` describes it through `aria-describedby`.
...
A radio list item used inside `RadioGroup.List` for connected list-style radio options with borders and padding. Inside `Field.Control`, the item picks up `aria-invalid`, `aria-errormessage`, and `aria-describedby`. A `Choice.Title` inside names it through `aria-labelledby`, and a `Choice.Description` describes it through `aria-describedby`.
...
A radio card item with enhanced styling for card-based radio options. Inside `Field.Control`, the item picks up `aria-invalid`, `aria-errormessage`, and `aria-describedby`. A `Choice.Title` inside names it through `aria-labelledby`, and a `Choice.Description` describes it through `aria-describedby`.
