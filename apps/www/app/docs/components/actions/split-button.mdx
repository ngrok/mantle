...
All props from [`Button`](/components/actions/button), except `appearance` and `priority`.
...
All props from [`IconButton`](/components/actions/icon-button), except `appearance`, `size`, `asChild`, and `icon` (which is redefined as optional below).
