...
All props from [`Button`](/components/actions/button), except `appearance`, `priority`, and `size` (set `size` on `SplitButton.Root` instead).
...
All props from [`IconButton`](/components/actions/icon-button), except `appearance`, `size` (set `size` on `SplitButton.Root` instead), `asChild`, and `icon` (which is redefined as optional below).
