...
`QrCode.Frame` is the image a screen reader meets. It always carries `role="img"`. Pass an `aria-label` (or `aria-labelledby`) that says what the code encodes, so the reader knows what a scan does. When neither is set, the name falls back to "QR code".
...
An `Overlay` logo is decorative: the frame's name already describes the code.
...
pattern to fit via its `viewBox`. Renders an `svg` with `role="img"`. Pass
`aria-label` or `aria-labelledby` to name it; the name falls back to "QR code".
Does not support `asChild`.

Accepts all props from [svg](https://developer.mozilla.org/en-US/docs/Web/SVG/Element/svg#attributes) except `role`.

### QrCode.Pattern

The encoded modules, rendered as a single `path` inside the `Frame`. Defaults
to a black fill; a `className` can override it, but a light or inverted fill
makes the code unreliable for many scanners. Renders a `path`. Does not
support `asChild`.
...
| -------------- | ------------------- | ----------------------------- |
| `data-slot`    | `"qr-code"`         | The `Root` tile.              |
| `data-slot`    | `"qr-code-frame"`   | The `Frame` svg.              |
| `data-slot`    | `"qr-code-pattern"` | The `Pattern` path.           |
| `data-slot`    | `"qr-code-overlay"` | The `Overlay` logo container. |
