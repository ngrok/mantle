...
Screen readers do not announce symbol-only glyphs (`⌘`, `⌃`, `↵`) meaningfully. Put a visually hidden label inside the `<Kbd>` and mark the visible glyph `aria-hidden`. Do not set `aria-label` on the `<Kbd>`: `<kbd>` has the `generic` role, which [prohibits naming](https://www.w3.org/TR/wai-aria-1.2/#generic), so a screen reader does not read the label reliably.
