...
- **Bracket-paired (Go / Rust / Java / C# / Terraform)**: multi-line `{ … }` / `[ … ]` ranges fold via TextMate-scope-aware token walking; brackets inside strings, comments, or regular expressions are ignored.
...
← [Back to Code Block](/components/data-display/code-block)
...
HCL blocks (`resource "type" "name" { … }`), object values, and multi-line lists all fold as bracket pairs.
