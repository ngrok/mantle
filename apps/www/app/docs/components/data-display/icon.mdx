...
An `Icon` is decorative by default. It renders `aria-hidden="true"` unless you pass `aria-label`, `aria-labelledby`, `role`, or an explicit `aria-hidden`, on the `Icon` or on the svg element itself. Phosphor icons set none of these, so an icon next to a text label is hidden from assistive technology and the text carries the meaning.
