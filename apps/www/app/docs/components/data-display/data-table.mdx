...
| `appearance?`   | `"ghost" \| "outlined"`             | `"ghost"`                    | The visual style of the underlying [`IconButton`](/components/actions/icon-button). Optional here — the expand toggle's design is a ghost button, so the wrapper defaults it. |
