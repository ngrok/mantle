...
Code blocks render and apply syntax highlighting to blocks of code. The [`mantleCodeBlockPlugins()`](/vite-plugins#mantlecodeblockplugins) Vite plugin highlights the code at build time with [Shiki](https://shiki.style/), so there is zero highlighting cost in the browser.
...
For frontends backed by non-Node.js services (Go, Python, etc.), deploy a small Node.js highlight service as a sidecar or shared microservice. This gives you the same server-rendered highlighting without shipping Shiki to the browser; the frontend calls the API with `fetch`.
...
For one example per supported language plus a list of caveats, see [folding by language](/components/data-display/code-block/folding-by-language).
...
For lower-level control, `jsonToShikiHtml(code)` returns only the Shiki-identical inner HTML for a `<code>` element (no line-number or fold decoration); `jsonCodeBlockValue` builds on it.
...
- `CodeBlock.CopyButton` is an icon button named by `label` (default "Copy code"). After a copy, an always-mounted `role="status"` element beside the button announces "Copied", so a screen-reader user hears the result and not only the icon swap.
- `CodeBlock.ExpanderButton` carries `aria-expanded` and `aria-controls` that points at the `<pre>`. The `<pre>` itself carries no ARIA state; `data-state="collapsed" | "expanded"` drives the styling.
- Fold toggles are real `<button>` elements named "Toggle code folding" with `aria-expanded`. Enter and Space toggle them. The folded marker is `aria-hidden`.
- Line numbers are `aria-hidden`, so a screen reader reads the code, not "1 const x".
- `CodeBlock.Code` and `CodeBlock.Title` carry `translate="no"`; see [Translation](#translation).
...
Inside the highlighted HTML, `data-slot="line-number"`, `data-slot="fold-toggle"`, and `data-slot="fold-ellipsis"` mark the gutter number, the fold button, and the folded marker.
...
The optional copy button. Render it as a child of `CodeBlock.Body`. It copies the raw source of the mounted `CodeBlock.Code` to the clipboard — in a tabbed block, the active tab's code.
...
| Prop           | Type                       | Default       | Description                                                                                                                                         |
| :------------- | :------------------------- | :------------ | :-------------------------------------------------------------------------------------------------------------------------------------------------- |
| `label?`       | `string`                   | `"Copy code"` | The accessible name for the copy button, rendered as its `aria-label`.                                                                              |
...
After a copy, an always-mounted `role="status"` element inside the part announces "Copied".

### CodeBlock.ExpanderButton

The (optional) expander button of the `CodeBlock`. Render this as a child of the `CodeBlock.Root` to allow users to expand/collapse the code block contents. It carries `aria-expanded` and `aria-controls` that points at the `<pre>`. It does not take `asChild`: it renders its own label and caret, so style it with `className`.

All props from [standard HTML button attributes](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/button#attributes) except `children`, `aria-controls`, and `aria-expanded`.
...
All props from [Icon](/components/data-display/icon), plus:
...
A tab list for the `CodeBlock` header. Renders pill-styled tab triggers that switch which code is displayed. Place this inside `CodeBlock.Header` and pair with `CodeBlock.TabContent` in `CodeBlock.Body`. `CodeBlock.Root` owns the tab state via `defaultTab` / `activeTab` / `onActiveTabChange`.
