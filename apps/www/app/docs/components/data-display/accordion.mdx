	...
	return (
		<div className="flex w-full flex-col gap-3">
			...
			<p className="text-muted text-sm">
				Open section: <code translate="no">{value || "none"}</code>
			</p>
			<div className="flex gap-2">
				...
				<Button type="button" appearance="outlined" intent="neutral" onClick={() => setValue("")}>
					Collapse all
				</Button>
			</div>
		</div>
...
find-in-page (<kbd translate="no">⌘F</kbd> / <kbd translate="no">Ctrl+F</kbd>) can find text inside a closed section and automatically expand it.
		...
		<Accordion.Item value="dns">
			...
			<Accordion.Content>
				<Accordion.Body>
					...
					reserved domain, then start your tunnel with the matching{" "}
					<code translate="no">--domain</code> flag.
				</Accordion.Body>
			</Accordion.Content>
		</Accordion.Item>
	...
	<Accordion.Item value="dns">
		...
		<Accordion.Content>
			<Accordion.Body>
				...
				domain, then start your tunnel with the matching <code translate="no">--domain</code> flag.
			</Accordion.Body>
		</Accordion.Content>
	</Accordion.Item>
	<Accordion.Item value="webhooks">
...
> [!NOTE]
> Collapse every section above, then press <kbd translate="no">⌘F</kbd> and search for a word that only appears inside a closed
...
- Switching between mutually exclusive views of the same region: use [`Tabs`](/components/navigation/tabs) instead.
- A single, always-relevant block of content: don't hide it behind a toggle; render it.
...
each header. Because the `Item` is a plain `<div>` and the `Trigger` is a real `<button>`, the header is a flex
	...
	>
		<Accordion.Item value="on_tcp_connect">
			<div className="mx-4 flex items-center gap-2">
				...
				<Button type="button" appearance="link" intent="accent" icon={<PlusIcon />}>
...
`Accordion.Trigger` is a real `<button>` whose `aria-expanded` reflects its section's open state and whose
`aria-controls` points at its `Accordion.Content`. Because the trigger is a genuine button, keyboard support is the
browser's own; there is no custom key handling to get wrong:
...
Collapsed content stays in the DOM but leaves the accessibility tree and the tab order, like a closed `<details>`. In
browsers that support `hidden="until-found"`, the content carries that attribute, so the browser's find-in-page can
still match and reveal it. In other browsers, and in the server HTML, the content carries `inert` instead. The
open/close slide respects `prefers-reduced-motion`.
...
`Accordion.Root` accepts an `asChild` prop. When `true`, the root renders its single child element instead of the default `<div>`, forwarding its class names, `data-*` attributes, and `ref` onto that child. Reach for it when the accordion should be a semantic landmark such as a `<section>`.

<Example className="flex-col gap-6">
	<Accordion.Root asChild defaultValue={["dns"]} className="max-w-xl">
...
Pass `value` + `onValueChange` for controlled usage, or `defaultValue` for uncontrolled usage. Accepts all `<div>`
props, plus:
...
native `<summary>` carries, via a real button so the role is consistent across browsers. Its `aria-controls` points at
the item's `Accordion.Content`. Defaults to full-width with
...
[`Icon`](/components/data-display/icon) props (notably `className` and `style`), plus:
...
open and closed. When collapsed, it leaves the accessibility tree and the tab order: in browsers that support
`hidden="until-found"` it carries that attribute, so its text stays discoverable by find-in-page and a `beforematch`
listener opens the section when the browser reveals a match; in other browsers, and in the server HTML, it carries
`inert`. Its `id` is the target of the trigger's `aria-controls`; `Accordion.Item` generates it, so `id` is not a prop
here. It holds no padding of its own so its `h-0 ↔ h-auto` slide can collapse to zero height: put the section's content
inside an `Accordion.Body`. It is flow content, so it may contain **anything**, including interactive elements
(buttons, links, inputs) or a nested `Accordion`. Accepts all `<div>` props except `id`.
