...
`Accordion.Root` accepts an `asChild` prop. When `true`, the root renders its single child element instead of the default `<div>`, forwarding its class names, `data-*` attributes, and `ref` onto that child — useful when the accordion should be a semantic landmark such as a `<section>`.
