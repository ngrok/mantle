---
title: Icons
description: Custom icons used throughout ngrok UI.
---

import {
	AutoThemeIcon,
	NgrokLettermarkIcon,
	NgrokWordmarkIcon,
	SortIcon,
	ThemeIcon,
	TrafficPolicyFileIcon,
} from "@ngrok/mantle/icons";
import { Example } from "~/components/example";
import { IconsExplorer } from "~/features/icons-explorer";

# Icons

A list of custom icons that are used throughout ngrok UI.

<IconsExplorer />

<Example className="gap-4">
	<SortIcon mode="time" direction="newest-to-oldest" />
	<ThemeIcon theme="system" />
	<AutoThemeIcon />
	<TrafficPolicyFileIcon />
	<NgrokLettermarkIcon />
	<NgrokWordmarkIcon />
</Example>

```tsx
import {
	AutoThemeIcon,
	NgrokLettermarkIcon,
	NgrokWordmarkIcon,
	SortIcon,
	ThemeIcon,
	TrafficPolicyFileIcon,
} from "@ngrok/mantle/icons";

function IconGallery() {
	return (
		<>
			<SortIcon mode="time" direction="newest-to-oldest" />
			<ThemeIcon theme="system" />
			<AutoThemeIcon />
			<TrafficPolicyFileIcon />
			<NgrokLettermarkIcon />
			<NgrokWordmarkIcon />
		</>
	);
}
```

## API Reference

All custom icons are exported from `@ngrok/mantle/icons`.

### SortIcon

A sort direction indicator icon with different modes and directions.

The props are a discriminated union on `mode`: each mode accepts its own `direction` values.

| Prop        | Type                                                          | Default | Description                                                                                              |
| :---------- | :------------------------------------------------------------ | :------ | :------------------------------------------------------------------------------------------------------- |
| `mode`      | `"alphanumeric"`                                              |         | Sort by alphanumeric order.                                                                              |
| `direction` | `"asc" \| "desc"`                                             |         | `"asc"` for A to Z and 0 to 9, `"desc"` for the reverse.                                                 |
| `mode`      | `"time"`                                                      |         | Sort by time.                                                                                            |
| `direction` | `"newest-to-oldest" \| "oldest-to-newest" \| "asc" \| "desc"` |         | `"newest-to-oldest"` (or `"desc"`) for newest first, `"oldest-to-newest"` (or `"asc"`) for oldest first. |

### TrafficPolicyFileIcon

The ngrok traffic policy file icon.

### AutoThemeIcon

An icon that automatically adapts to the current applied theme.

### ThemeIcon

An icon for a specific theme.

| Prop    | Type                                                                             | Default | Description           |
| :------ | :------------------------------------------------------------------------------- | :------ | :-------------------- |
| `theme` | `"system" \| "light" \| "dark" \| "light-high-contrast" \| "dark-high-contrast"` |         | The theme to display. |

### NgrokWordmarkIcon

An inline SVG of the ngrok wordmark logo; fill color follows the `color` CSS property.

### NgrokLettermarkIcon

An inline SVG of the ngrok lettermark "n" logo; fill color follows the `color` CSS property.

## Icon reference

This table is intentionally duplicated in plain markdown so readers (and AI agents) consuming the raw `.mdx` source can get full icon context without JSX rendering.

| Icon                    | Description                                                                                         |
| :---------------------- | :-------------------------------------------------------------------------------------------------- |
| `SortIcon`              | Sort direction indicator icon with `mode` (`"alphanumeric"` or `"time"`) and `direction` variants.  |
| `TrafficPolicyFileIcon` | The ngrok traffic policy file icon.                                                                 |
| `AutoThemeIcon`         | Icon that automatically adapts to the currently applied theme.                                      |
| `ThemeIcon`             | Theme-specific icon controlled by the `theme` prop (`"system"`, `"light"`, `"dark"`, and HC modes). |
| `NgrokWordmarkIcon`     | An inline SVG of the ngrok wordmark logo; fill color follows the `color` CSS property.              |
| `NgrokLettermarkIcon`   | An inline SVG of the ngrok lettermark "n" logo; fill color follows the `color` CSS property.        |
