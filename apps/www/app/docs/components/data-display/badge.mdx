...
| `data-slot`    | `"badge-label"` | On the `<span>` wrapping `children`, beside the `icon` slot. `display: contents`, so every child stays a flex item of the badge and the `gap` still falls between them. To style the label as a box, give the slot a display first, with `[&>[data-slot=badge-label]]:block`. |
