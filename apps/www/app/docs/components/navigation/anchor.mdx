...
- Internal application routes — prefer the framework router's link primitive ([`react-router` `<Link>`](https://reactrouter.com/en/main/components/link)) so client-side navigation kicks in. You can keep mantle's styling by composing — see [Polymorphism](#polymorphism).
...
Link text must be self-describing — avoid "click here" / "read more". For purely decorative icons, no extra labeling is needed; for icon-only links, pass an `aria-label`.
