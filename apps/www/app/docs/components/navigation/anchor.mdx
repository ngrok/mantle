...
Link text must be self-describing: avoid "click here" / "read more". The icon renders `aria-hidden`, so it needs no labeling; for icon-only links, pass an `aria-label`.
