...
The tab list's bottom border is composed: render a `Tabs.ListBorder` as a child of `Tabs.List` to draw a 1px border in the `separator` color that terminates at the ends of the tab triggers, and omit it to render no border. The pill appearance never draws a border, so `Tabs.ListBorder` is always safe to compose.
