...
A set of layered sections of content—known as tab panels—that are displayed one at a time. The outermost part; its context carries `orientation` and `appearance` to every other part. Based on [Radix Tabs Root](https://www.radix-ui.com/primitives/docs/components/tabs#root).
