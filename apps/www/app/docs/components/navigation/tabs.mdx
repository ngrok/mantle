...
A horizontal classic tab list draws a 1px bottom border in the `separator` color by default. Pass `hideBorder` to `Tabs.List` to remove it (a vertical classic tab list's side border is removed the same way). The prop is also rendered as a `data-hide-border` attribute on the tablist element. The pill appearance never draws a border, so `hideBorder` has no effect there.

<Example className="grid gap-6">
	<div>
		<p className="mb-2 font-mono text-xs">default</p>
		<div className="scrollbar -m-1 p-1 overflow-x-scroll">
			<Tabs.Root orientation="horizontal" defaultValue="tab-1">
				<Tabs.List>
					<Tabs.Trigger value="tab-1">Tab Title</Tabs.Trigger>
	...
	<div>
		<p className="mb-2 font-mono text-xs">hideBorder</p>
		<div className="scrollbar -m-1 p-1 overflow-x-scroll">
			<Tabs.Root orientation="horizontal" defaultValue="tab-1">
...
| ------------------ | ------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------- |
| `data-slot`        | `"tabs"` \| `"tabs-list"` \| `"tabs-trigger"` \| `"tabs-trigger-label"` \| `"tabs-badge"` \| `"tabs-content"` | Names each part, plus the trigger's label wrapper.      |
| `data-orientation` | `"horizontal"` \| `"vertical"`                                                                                | On the root, list, triggers, and content. Set by Radix. |
| `data-appearance`  | `"classic"` \| `"pill"`                                                                                       | On the root. Descendant parts style against it.         |
| `data-hide-border` | present when `hideBorder`                                                                                     | Presence-only, on the list.                             |
...
By default a classic tab list draws a 1px border in the `separator` color — the [bottom border](#bottom-border) when horizontal, a side border when vertical. The pill appearance never draws a border.

| Prop          | Type      | Default | Description                                                                                                                                                           |
| ------------- | --------- | ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `hideBorder?` | `boolean` | `false` | Hide the tab list's border. Also rendered as a `data-hide-border` attribute on the tablist element. Has no effect on the pill appearance, which never draws a border. |
