...
A horizontal classic tab list draws a 1px bottom border in the `separator` color by default; it terminates at the ends of the tab triggers. Pass `hideBorder` to `Tabs.List` to remove it (a vertical classic tab list's side border is removed the same way). The prop is also rendered as a `data-hide-border` attribute on the tablist element. The pill appearance never draws a border, so `hideBorder` has no effect there.
