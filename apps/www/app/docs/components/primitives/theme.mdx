...
export function ThemeSwitcherExample() {
    ...
    return (
    	...
    	>
    		<Select.Trigger className="w-min" aria-label="Theme Switcher">
    			<SunIcon className="mr-1 h-6 w-6" />
    		</Select.Trigger>
    		<Select.Content>
    			<Select.Group>
...
Mantle's theme system ships `ThemeProvider` (context + cookie persistence), `MantleStyleSheets` (lazy-loaded dark/HC CSS via `<link media>`), and `PreventWrongThemeFlashScript` (FOUC prevention inline script) for light, dark, and high-contrast modes.
...
export function Layout({ children }: PropsWithChildren) {
	...
	return (
		<html {...initialHtmlThemeProps} lang="en-US" dir="ltr" suppressHydrationWarning>
			...
			<body>
				<ThemeProvider ssrCookie={loaderData?.ssrCookie}>{children}</ThemeProvider>
			</body>
		</html>
	);
}
...
Both generators take the same optional forced theme: `preventWrongThemeFlashScriptContent("light")` and `fixMediaScriptContent("light")`. Pass it to both or neither; see [Locking a page to one theme](#locking-a-page-to-one-theme).
...
> [!NOTE]
> `preventWrongThemeFlashScriptContent()` and `fixMediaScriptContent()` from `@ngrok/mantle/theme` generate these script strings. The scripts above are their output for the version of mantle this page was built with, so you can copy them as they are. Run the generators again after you upgrade mantle to pick up any changes.

## Usage

Use the `useTheme` hook to get and change the current theme:
...
## Inverted theme islands

Add the `invert-theme` class to any element to render that subtree in the opposite theme of the
page: light ⇄ dark and light-high-contrast ⇄ dark-high-contrast. The island is styled by the
opposite theme's own definitions, so every mantle component and color token inside it just works:
no conditional rendering, no per-component overrides.

```tsx
import { Button } from "@ngrok/mantle/button";

<aside className="invert-theme bg-base text-strong rounded-2xl p-4 shadow-lg">
	<p>This panel renders dark on light pages, light on dark pages.</p>
...
[Sandbar](/components/feedback/sandbar) is built on this: its floating panel is an `invert-theme`
island, which is why the bar and its buttons always read inverted from the page in all four
themes.

How it works: every mantle color utility resolves a CSS custom property at the element that uses
it, and custom properties inherit, so each theme's definitions are also applied directly to
`.invert-theme` elements on its pair partner's pages, re-declaring the theme variables where they
win over the values inherited from `:root`.

Things to know:

- **Requires the theme stylesheets.** `MantleStyleSheets` applies theme pairs together (the
  active theme's partner also gets `media="all"`), so inversion works automatically. If you load
  the theme CSS by hand, both pair members' stylesheets must be applied. Like manual theme
  selection generally, the pairing on OS-default pages is applied by the inline fix script; with
  JavaScript disabled, islands gracefully stay page-themed.
- **Portals escape the island.** Floating content that portals to `<body>` (dropdown menus,
  tooltips, dialogs, …) renders in the page theme. Pass `invert-theme` in the floating part's
  `className` to invert it too.
- **Theme variant utilities do not flip.** `dark:*`, `light:*`, `high-contrast:*`, and
  `dark-high-contrast:*` key off the page theme on `<html>`, not the inversion scope; this
  applies to your own utilities as much as to mantle's. Six mantle internals still carry small
  variant-gated tweaks (Progress Bar, Progress Donut, RadioGroup, Skeleton, Table, Toast).
  Avoid composing those inside an island until they migrate to theme-declared tokens.
- **Rebrand at the ramp, not the alias.** Islands re-declare the semantic aliases
  (`--color-accent-*`, `--color-danger-*`, …) on themselves so they re-resolve against the
  inverted ramps, and a declaration on the element beats an inherited value. Override
  `--color-sky-600` rather than `--color-accent-600` and the change flows into islands too.
- **Nesting is idempotent.** An `invert-theme` inside an inverted subtree stays inverted relative
  to the page; it does not revert to the page theme.
- **Rebranding a ramp does not reach islands.** An island restores mantle's pinned Tailwind
  defaults, so a consumer `@theme` palette override applies to the page but not inside an island.
  Closing that would need a shadow copy of every ramp under a second name.
- **Shadows flip with the island.** Shadow utilities resolve the shadow tokens at the element
  that uses them, and islands re-declare those tokens, so content inside a light island gets
  light-theme shadows, keeping the island's interior coherent.

## Locking a page to one theme

`forceTheme` is not one prop: it is the same value passed to every piece that decides what the
page renders in. Two independent things have to agree:

1. **Which stylesheets are applied**: owned by `MantleStyleSheets` (and its inline fix script).
2. **Which theme class is on `<html>`**: owned by `PreventWrongThemeFlashScript` before first
   paint, by `ThemeProvider` after mount, and by `useInitialHtmlThemeProps` in the SSR'd markup.

Forcing only the first is a trap. Because theme pairs are applied together, a `forceTheme="light"`
page has `mantle-dark.css` live; if the stored preference resolves to dark, the writers above put
`class="dark"` on `<html>`, `:root.dark` outranks the light base, and the page paints dark, the
opposite of what was asked for. Pass the same value everywhere:
...
export default function Root() {
	const htmlProps = useInitialHtmlThemeProps({ forceTheme: FORCED_THEME, ssrCookie });

	return (
		<html {...htmlProps}>
			<head>
				<PreventWrongThemeFlashScript forceTheme={FORCED_THEME} nonce={nonce} />
				<MantleStyleSheets forceTheme={FORCED_THEME} {...themeUrls} nonce={nonce} />
			</head>
			<body>
				<ThemeProvider forceTheme={FORCED_THEME} ssrCookie={ssrCookie}>
					<App />
				</ThemeProvider>
			</body>
		</html>
...
The stored preference is still read, and `setTheme` still writes it, so a theme switcher keeps
working and other pages honor the user's choice. Only what this page paints is pinned:
`data-applied-theme` and the class are the forced theme, while `data-theme` remains the preference.

That divergence is safe by construction. Every theme block guards its `data-theme` selector with
`:not([data-applied-theme])`, so once anything applies a theme, the preference stops selecting one.
Set `data-theme` on its own and it still picks the theme, exactly as before.
...
| `forceTheme?` | `ResolvedTheme` |         | Pin the theme that renders, ignoring the stored preference and the OS. See [Locking a page to one theme](#locking-a-page-to-one-theme) — pass the same value to every component that forces a theme.                                                                                                               |
| `ssrCookie?`  | `string`        |         | The theme cookie pair from the request, via `extractThemeCookie`. On the server, the provider seeds the theme from it, so `useTheme` and `useAppliedTheme` hydrate without a mismatch. When omitted, the server renders `system`. Pass the same value you pass `MantleStyleSheets` and `useInitialHtmlThemeProps`. |

The `ThemeProvider` uses a hardcoded storage key of `mantle-ui-theme` and defaults to `system` theme. The provider manages both values internally, so they need no configuration.
...
Theme pairs are applied together: the active theme's pair partner (light ⇄ dark, light-high-contrast ⇄ dark-high-contrast) also gets `media="all"`, so [`invert-theme` islands](#inverted-theme-islands) always have their opposite theme's CSS available. Light is the base theme in `mantle.css` and needs no link tag of its own.

When `forceTheme` is set, only the link tags for that theme's pair are rendered — the others are omitted to avoid unnecessary network requests. `forceTheme="light"` renders only the dark link (its pair partner). Because the partner is _applied_, this prop controls only half the picture — see [Locking a page to one theme](#locking-a-page-to-one-theme).
...
| `forceTheme?`             | `ResolvedTheme` |         | Force a specific theme's stylesheet pair to load unconditionally (`media="all"`). Use for pages locked to a single theme (e.g. a dark-only landing page). Only the forced theme's pair of `<link>` tags is rendered; the others are omitted. `"light"` renders only the dark link (its pair partner). |
...
| Prop          | Type            | Default | Description                                                                                                             |
| ------------- | --------------- | ------- | ----------------------------------------------------------------------------------------------------------------------- |
| `forceTheme?` | `ResolvedTheme` |         | Pin the theme this page paints in, before first paint. See [Locking a page to one theme](#locking-a-page-to-one-theme). |
...
Extracts only the theme cookie key-value pair from a full `Cookie` header string. Pass the result to `ThemeProvider`, `MantleStyleSheets`, and `useInitialHtmlThemeProps` as `ssrCookie`. This keeps the other cookies from the request out of loader data.
...
| Prop          | Type            | Default | Description                                                                                                                                                                                         |
| ------------- | --------------- | ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `className?`  | `string`        | `""`    | Additional CSS classes to apply to the `<html>` element. The hook combines these with the theme class.                                                                                              |
| `forceTheme?` | `ResolvedTheme` |         | Pin the resolved theme in the returned props. See [Locking a page to one theme](#locking-a-page-to-one-theme).                                                                                      |
...
On the server, when `ssrCookie` is set, the hook reads the theme from the cookie to render the correct class. When omitted, it defaults to `system` theme (resolved to `light`). On the client, it reads the current theme from `document.cookie` to match what the FOUC prevention script applied, so React hydration succeeds.
