...
> [!NOTE]
> `preventWrongThemeFlashScriptContent()` and `fixMediaScriptContent()` from `@ngrok/mantle/theme` generate these script strings. Re-run them after you upgrade mantle to pick up any changes.
