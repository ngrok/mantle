...
[PowerBar](/components/feedback/power-bar) is built on this: its floating panel is an `invert-theme`
...
- **Theme variant utilities do not flip.** `dark:*`, `light:*`, `high-contrast:*`, and
  `dark-high-contrast:*` key off the page theme on `<html>`, not the inversion scope. A few
  mantle internals still carry small variant-gated tweaks (Table, Skeleton, Progress) — avoid
  composing those inside an island until they migrate to theme-declared tokens.
...
When `forceTheme` is set, only the link tags for that theme's pair are rendered — the others are omitted to avoid unnecessary network requests. `forceTheme="light"` renders only the dark link (its pair partner).
