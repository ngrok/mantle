...
`SandboxedOnClick` accepts an `asChild` prop. When `true`, it renders its single child instead of its default `<div>`, forwarding the sandboxed `onClick` handler, `role="presentation"`, and the ref onto that child. Reach for this when the sandboxed click behavior should live directly on a semantic element such as an anchor, without an extra wrapper `<div>`.
