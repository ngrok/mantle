...
The focus call passes `preventScroll: true`, so the viewport does not move. Place the link at the top of the document and keep the target near the top of the viewport, as an app shell's main region is. A target below the fold takes focus without any visible change.
