...
The route is also available as plain markdown at [`/recipes/sheet-async.md`](/recipes/sheet-async.md), so humans and LLM tools can fetch the same recipe without scraping the rendered page.
