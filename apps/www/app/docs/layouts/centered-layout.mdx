...
- **`Notice`** is a full-window-width strip pinned above everything else — including `Header` — for impersonation notices, environment warnings, and similar app-wide messaging. It is unstyled (`w-full shrink-0`): the notice content brings its own colors, and the part collapses to nothing when empty. The same slot contract as [`AppLayout.Notice`](/layouts/app-layout#applayoutnotice), so an app-wide banner composes identically across mantle's layouts — see [Notice](#notice). Optional.
