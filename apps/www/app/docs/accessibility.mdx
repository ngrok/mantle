...
- **Semantic HTML first.** A [`Button`](/components/actions/button) is a `<button>`. A [`Main`](/components/primitives/main) is a `<main>`. Form controls are real `<input>` / `<select>` / `<textarea>` elements. See [Philosophy](/philosophy#semantic-html-then-everything-else) for the full argument.
...
1. Mount [`<ThemeProvider>`](/components/primitives/theme), [`<TooltipProvider>`](/components/overlays/tooltip), and [`<Toaster>`](/components/feedback/toast) at the root of the document tree.
2. Place [`<SkipToMainLink />`](/components/primitives/skip-to-main-link) as the very first focusable element in `<body>`, paired with a [`<Main>`](/components/primitives/main) landmark wrapping the page's primary content.
3. Render a single `<h1>` per page that names the page.
4. Use real form elements with associated [`<Label>`](/components/forms/label)s.
5. Set accessible names on icon-only controls — [`IconButton`](/components/actions/icon-button) requires a `label` prop.
...
| Component                                                                                                                   | Keys                                                                                                                                              |
| --------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| [Button](/components/actions/button) / [IconButton](/components/actions/icon-button)                                        | `Tab` to focus, `Enter` or `Space` to activate.                                                                                                   |
| [Anchor](/components/navigation/anchor)                                                                                     | `Tab` to focus, `Enter` to follow.                                                                                                                |
| [Checkbox](/components/forms/checkbox) / [Switch](/components/forms/switch)                                                 | `Tab` to focus, `Space` to toggle.                                                                                                                |
| [RadioGroup](/components/forms/radio-group)                                                                                 | `Tab` to enter the group, `Arrow` keys to move and select within the group.                                                                       |
| [Tabs](/components/navigation/tabs)                                                                                         | `Tab` to enter the tab list, `Arrow Left`/`Right` to move, `Home`/`End` to jump, `Enter`/`Space` to activate.                                     |
| [Dialog](/components/overlays/dialog) / [AlertDialog](/components/overlays/alert-dialog)                                    | Focus is trapped inside while open, `Esc` closes (Dialog only — AlertDialog requires explicit confirmation), `Tab`/`Shift+Tab` cycles focus.      |
| [Sheet](/components/overlays/sheet)                                                                                         | Same as Dialog.                                                                                                                                   |
| [Popover](/components/overlays/popover) / [HoverCard](/components/overlays/hover-card)                                      | `Esc` closes; `Tab` cycles focusable content. HoverCard opens on hover/focus and is non-essential.                                                |
| [DropdownMenu](/components/overlays/dropdown-menu) / [Command](/components/navigation/command)                              | `Arrow Up`/`Down` to navigate items, `Enter` to select, `Esc` to close, type-ahead jumps to matching items.                                       |
| [Combobox](/components/forms/combobox) / [MultiSelect](/components/forms/multi-select) / [Select](/components/forms/select) | `Arrow Up`/`Down` through options, `Enter` selects, type filters, `Esc` closes.                                                                   |
| [Slider](/components/forms/slider)                                                                                          | `Arrow Left`/`Right` (or `Up`/`Down`) increment/decrement, `Home`/`End` jump to min/max, `PageUp`/`PageDown` for larger steps.                    |
| [DataTable](/components/data-display/data-table)                                                                            | Row click handlers respond to `Enter` when used with `asChild` rendering of an `<a>` or `<button>`. Header sort buttons follow Button keys above. |
| [Toast](/components/feedback/toast)                                                                                         | Auto-dismissing toasts respect `prefers-reduced-motion`; `F8` (browser default) cycles to the toast region.                                       |
...
- **Visible focus rings.** Mantle uses `:focus-visible` so focus styles appear for keyboard users without distracting mouse users. Don't override `outline` without setting an equivalent ring.

## ARIA roles and labels

Mantle components emit appropriate roles automatically. Consumers need to add:

- `label` on [`IconButton`](/components/actions/icon-button), which it renders as visually-hidden text. `aria-label` on any [`Button`](/components/actions/button) whose content is icon-only.
- `aria-label` or visually-hidden text on [`Kbd`](/components/data-display/kbd) when the visible glyph is a symbol (e.g. `⌘`).
- `aria-describedby` linking error/help text to form controls. [`Input`](/components/forms/input) and friends do this for you when you use the validation prop, but custom messages should be wired manually.
- `aria-current="page"` on the active item in primary navigation (use react-router's `NavLink` for this).
- Live-region semantics — use [`Toast`](/components/feedback/toast) for transient announcements; use the [`Alert`](/components/feedback/alert) component with `role="alert"` for inline warnings.
...
- High-contrast variants ship as dedicated stylesheets — import [`@ngrok/mantle/mantle-light-high-contrast.css`](https://www.npmjs.com/package/@ngrok/mantle?activeTab=code) or `@ngrok/mantle/mantle-dark-high-contrast.css` to opt in.
...
3. Add a fallback path (e.g. a plain `<a>` with the same target) so keyboard and screen-reader users are not blocked.
