...
- `label` on [`IconButton`](/components/actions/icon-button), which it renders as `aria-label`. `aria-label` on any [`Button`](/components/actions/button) whose content is icon-only.
