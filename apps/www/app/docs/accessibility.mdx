...
5. Set accessible names on icon-only controls — [`IconButton`](/components/actions/icon-button) requires `aria-label`.
...
- `aria-label` on [`IconButton`](/components/actions/icon-button), and any [`Button`](/components/actions/button) whose label is icon-only.
