...
and snaps in engines without it. [SkipToMainLink](/components/primitives/skip-to-main-link)
...
they emit, so the API tells the truth about the markup: `List.Item`'s `current` prop
sets `aria-current`. And where WAI-ARIA offers competing patterns, the component makes the
...
Because names promise behavior, ARIA's pattern words are earned rather than borrowed. A
`Menu` owes you roving focus, typeahead, `Escape` to dismiss, and a single tab stop, so
only a real `role="menu"` component gets that name. [DropdownMenu](/components/overlays/dropdown-menu)
has it; a `<ul>` of navigation links doesn't, however many other libraries call that
markup a menu. The same goes for `Listbox`, `Tab`, `Tree`, `Grid`, and `Combobox`. When a
name and the markup disagree, we rename the part. We never add the role to justify the
name; that's how a perfectly good list of links loses its tab order. Mechanics are worth
porting from the ecosystem; vocabulary is re-derived from what the component emits.

## The right foundation, per problem

Mantle isn't "built on Radix." Where the platform is enough, components are native:
...
You can't tell which is which from the outside, and that's the point. Every foundation
...
"pick several" is [MultiSelect](/components/forms/multi-select). The JSDoc on each names
...
[Slot](/components/primitives/slot), swaps the rendered element entirely, so Mantle never forces
a DOM structure on you: internal navigation is `<Anchor asChild><Link to="…" /></Anchor>`,
not an `href` prop with router flags. Configuration props handle the common case;
composition handles everything else. Never a prop bag: a component doesn't forward
...
new color can't ship half-styled.

What the compiler can't catch fails loudly at runtime. Compound parts rendered outside
...
values and TypeScript types can't drift apart. Components obey the same layering:
...
The promise isn't that semantic markup makes accessibility free. The promise is that
...
`aria-describedby`, and `aria-errormessage` always resolve without hand-threading. Its
help affordance is a [Popover](/components/overlays/popover) rather than a tooltip, so touch
...
[`usePrefersReducedMotion`](/hooks) returns `true` on the server and in the hydration
render, so nothing animates until the user's real preference is known.
...
client-only code can't even evaluate during SSR. Every browser-API hook declares its
...
components with the same vocabulary you already know, so custom styling stays on-system
by default. [Colors](/base/colors) and [Typography](/base/typography) document the
vocabulary.

Your overrides always win. Every component composes consumer `className` last
...
time, so no highlighter JavaScript ships to the browser. Its React-free core is
...
across every caller. It caches its subscribe functions because unstable references
...
`Action` deliberately doesn't close the dialog, so destructive handlers must opt in to
...
we maintain examples with the same care as API. Every docs page ships a
...
open invitation to file issues. Breaking cleanups wait for majors. A principle you
