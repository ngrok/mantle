...
has it; a `<ul>` of navigation links does not, however many other libraries call that
markup a menu. The same goes for `Listbox`, `Tab`, `Tree`, `Grid`, and `Combobox`. When a
name and the markup disagree, we rename the part—never add the role to justify the name,
which is how a perfectly good list of links loses its tab order. Mechanics are worth
