...
The `$`-prefixed helpers are identity functions that return a runtime value with the correct inferred type. Useful for deriving typed values from string literals without a `as const` cast.
