...
Mantle's `mantle.css` already includes the Shiki CSS variable theme. Import it:
...
If you don't use MDX, disable it: `mantleCodeBlockPlugins({ mdx: false })`.
...
When `~preHtml` is omitted, CodeBlock falls back to rendering the code as escaped plain text (no colors, but still functional with copy button, line numbers, etc.).
...
CodeBlock uses Mantle's built-in CSS variable theme (`mantle-css-variables`). Import the Mantle CSS in your app:

```ts
import "@ngrok/mantle/mantle.css";
```

The Mantle `ThemeProvider` handles light/dark mode automatically. The highlighted HTML uses CSS variable class names that respond to the active theme.
...
The Vite plugin is not configured. `mantleCode` is a no-op at runtime — it requires the Vite plugin to transform tagged templates at build time. Add `mantleCodeBlockPlugins().vitePlugins` to your Vite config. In development, you may see a warning in the console.
...
`preHtml` is rendered via `dangerouslySetInnerHTML`. Only pass HTML produced by Shiki through the Vite plugin, `createMantleServerSyntaxHighlighter`, or the highlight server. Never pass HTML strings that came from a user.
