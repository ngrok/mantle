...
`DataTable.ActionHeader` applies the sticky positioning, the background, and the left-side fade indicator itself. It needs no other changes from you.
...
import { DataTable, createColumnHelper, tableFeatures } from "@ngrok/mantle/data-table";
...
- Tables with an action column that keep using the old `DataTable.Header` for the header will continue to render correctly; they won't get the aligned sticky header or the matching fade indicator on the header row.
