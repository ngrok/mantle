...
const otherKey = "other-test-preference";
...
describe("useLocalStorage", () => {
	...
	test("switching keys re-reads, resubscribes, and writes under the new key", () => {
		window.localStorage.setItem(key, JSON.stringify("first-key-value"));
		window.localStorage.setItem(otherKey, JSON.stringify("second-key-value"));

		const { result, rerender } = renderHook(
			({ activeKey }) => useLocalStorage(activeKey, defaultValue),
		...
		act(() => {
			window.localStorage.setItem(otherKey, JSON.stringify("updated"));
			window.dispatchEvent(
				new StorageEvent("storage", {
					key: otherKey,
		...
		expect(window.localStorage.getItem(otherKey)).toBe(JSON.stringify("written-after-switch"));
		expect(window.localStorage.getItem(key)).toBe(JSON.stringify("stale"));
	});
