...
- JSDoc: exported functions, hooks, components, and prop types are documented; required `@example` blocks are present; no `@param` or `@returns` only restates the name or the type.
...
- Prose: comments, JSDoc, changesets, decision docs, docs-page copy, the commit subject, and the PR description follow [CONVENTIONS.md § Writing](./CONVENTIONS.md#writing) — active voice, simple tenses, one idea per sentence, condition first, no comment that restates the code or runs past two sentences, and none of the banned filler (`utilize`, `facilitate`, `ensure`, `robust`, `comprehensive`, `simply`, and the six carve-out words).
- Tests: bug fixes have regression tests; business logic has edge-case tests (transformations, validation, conditional rendering, state machines, parsing/formatting). For every test you added or changed, name the one-line implementation change it would catch. If you cannot, it does not count. That answer is a wrong behavior, not an edit to the constant the test copies: no test asserts a constant against a copy of itself, and no test fails on a refactor that keeps the visible behavior ([CONVENTIONS.md § No tautological tests, no change-detector tests](./CONVENTIONS.md#no-tautological-tests-no-change-detector-tests)). Add a case only when it reaches a branch, a boundary, or a lookup entry no sibling reaches, and test shared logic once, where it lives. Anything interactive must be driven with a real event (`user.click` / `user.keyboard`) and assert the resulting DOM/ARIA state, not just initial markup. Mock the environment (`matchMedia`, the clipboard, a viewport hook), never a mantle part, and never assert that a mock received the value the test passed in. No Tailwind utility-string assertions, no arbitrary `setTimeout` waits, no `toBeDefined()`/`not.toThrow()` as a test's only assertion, no `getBy*` wrapped in `toBeDefined()`/`not.toBeNull()`, no bare `toHaveBeenCalled()` where the count matters. See [CONVENTIONS.md § Testing](./CONVENTIONS.md#testing).
