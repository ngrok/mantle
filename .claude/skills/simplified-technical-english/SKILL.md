...
2. Classify the text. Procedural prose (a commit subject, a step, a `TODO`) takes the imperative and 20 words. Descriptive prose (JSDoc, docs-page copy, a PR body) states the contract or the reason. Past 25 words is the smell. The limits do not reach `.changeset/*.md` or `decisions/*.md`.
...
| `// Hydration mismatch mitigation: defer the theme-dependent subtree to a post-commit effect to sidestep SSR nondeterminism.` | `// The server has no matchMedia, so this subtree renders only after mount.`                                                      |
| `/** Orchestrates measurement of the scroll container. @param options The options. @returns The result. */`                   | `/** Reports whether the list overflows its scrollport. @param options.threshold Pixels of overflow to ignore. Defaults to 0. */` |

## Self-check

- a banned word that no carve-out covers; remember the carve-outs: `Provider`, `enabled`, `performance`, `not just`, `high-leverage`, a `seamless` tile
