...
- Configurable point glyphs: `shape` (`"circle" | "square" | "triangle" | "diamond"`) on `ScatterPlot.Point` (marks, legend keys, hover dot), `LineChart.Line` (canvas markers + hover dot), and `AreaChart.Area` (hover dot) — a redundant encoding alongside color, sized for equal visual weight.
