...
The [breadcrumbs-from-routes recipe](https://mantle.ngrok.com/recipes/breadcrumbs-from-routes#the-prefix-crumb) now covers the prefix crumb: a layout route opts its crumb out of linking with `to: null`, and the renderer draws it as a `Breadcrumb.Label`.
