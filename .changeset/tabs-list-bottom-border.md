...
Restore the classic `Tabs.List` border, drawn by default in the `separator` color token: horizontal tablists draw a 1px bottom border and vertical tablists draw the matching side border (also the `separator` token now, previously `gray-200`). The border is painted on the list's content box so the focus-ring breathing padding doesn't push it past the container edges, it sits just below the active trigger's decoration, and it fades under the scroll-fade mask when the list scrolls.
